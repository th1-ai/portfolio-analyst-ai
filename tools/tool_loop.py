...
or a final answer. This is the same schema-constrained-JSON technique
ARCHITECTURE.md section 3 already documents for the `claude-code`/`anthropic`
providers - there is no native tool-calling in this family's LLM contract, so
the loop drives it by hand, exactly like the source system this was ported
from (see docs/how-it-works.md).

`provider=mock` still exercises the real tools: each round reads its own
fixture (`fixtures/expected/ask/<fixture_id>-r<n>.json`), so the
"model" is scripted but the arithmetic underneath it is not.
"""
...
class ToolLoopExhausted(RuntimeError):
    ...
    Carries whatever was actually tried (``tool_calls``, ``reports``) so a
    person reviewing the item can see what the Analyst attempted, not just
    that it gave up - see `tools/review.py`."""
...
def _replay_cached_rounds(item: Item) -> tuple[list[dict], list[dict], list[dict]]:
    """Rebuild ``tool_calls``/``reports`` from ``item.payload["_rounds"]`` with
    no LLM call and no tool re-execution - see the module docstring's note on
    resumability. Returns ``(cached_rounds, tool_calls, reports)``."""
...
def _cache_round(store: Store | None, item: Item, cached_rounds: list[dict],
    ...
    """Persist round ``round_no``'s result on the item payload so a restart
    replays it instead of asking the model (or re-running its tools) again.
    A no-op on ``--dry-run`` (``store is None``), same as every other write
    here - a rehearsal must never change what a real resume would see."""
    if store is None:
        return
...
def run_tool_loop(settings: Settings, store: Store | None, item: Item, question: str, ctx: ToolContext,
    ...
    `tools/engine.py` and turned into a `needs_human` item, never a crash.

    Resumable by design: every round that actually resolves (a live model
    call that returned "tools" or "final") is cached on
    ``item.payload["_rounds"]`` before the loop moves on. On the next call for
    the same item - the same `python3 tools/run.py --once --question "..."`
    re-run the `interactive` provider tells you to run, or any other restart
    mid-loop - those rounds are replayed from the cache with **no LLM call and
    no tool re-execution**; only the first round that was never cached talks
    to the model (or, under `interactive`, pends) again. See
    ARCHITECTURE.md/build-repo.md's resumable-stages note and
    `tools/engine.py:answer_question`'s docstring for why `_rounds` (an
    underscore-prefixed payload key) survives every `store.upsert_item`
    refresh untouched.
    """
    ...
    if start_round > max_rounds:
        # The cache alone already used the whole budget (e.g. max_rounds was
        # lowered after these rounds were cached) - fail the same way running
        # out live would, rather than replaying forever.
    ...
    for round_no in range(start_round, max_rounds + 1):
        ...
        prompt = build_prompt(
            ...
            persona_name=settings.agent_get("persona.name", "the Analyst"),
        ...
        if step.get("step") != "tools":
            continue  # an empty/unknown step is nudged by next round's prompt - nothing to cache
        ...
        # This round is fully resolved (its tools ran) - cache it now so a
        # restart never re-asks the model for it and never re-runs its tools.
